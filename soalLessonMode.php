<html>
<head>
<title>Lessons Mode Setting</title>
...
<script>
	$(document).ready(function(){
		...
		$("#btnUpdate").click(function() {
			...
			$.post("soalLessonMode_update.php",
			{
				isilesson:$("#txtIsiLesson").val()
			},
			function(data,status) {
				location.reload();
				alert("Data lesson mode has been updated");
				//alert(data);
			});
		});
...
<body background="typingadmin/image/bg.gif">
	...
	<?php
	$file = file_get_contents('lessonmode.txt', true);
	$arr = explode("\n", $file);
	...
	echo "<table border='1' align='center' rules=rows frame=box bgcolor='white' width='700px'>";
		...
		echo "<tr bgcolor='white' CLASS='darkrow'>";
			echo "<td align='center'>";
				echo "Upload and Download Lesson Mode";
			echo "</td>";
		echo "</tr>";
		...
		echo "<tr>
			<td>";
				echo "<input type='button' id='btnUpdate' value='Update lesson Mode' />";
	...
	function downloadtxt() {
		window.location = 'download_lessonsmode.php';
	}
	</script>
